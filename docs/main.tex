...
Tato kombinace poskytuje modelu extrémně silný prediktivní signál. Model díky předpovědi ví, jaká bude očekávaná celková spotřeba (např. vlivem ranní špičky nebo mrazivého počasí), a~zároveň díky týdennímu zpožděnému příznaku ($t-7$) dokáže korigovat systematické odchylky a~vnímat přirozený cyklus pracovních dnů a~víkendů v~českém průmyslu.
...
Po nasazení a~hyperparametrickém ladění algoritmu XGBoost došlo k~výraznému zpřesnění predikcí. Model dokázal stlačit chybovost na úroveň 13~EUR/MWh, čímž lineární regresi s~přehledem překonal. Pro lepší představu o~reálném dopadu této odchylky je důležité uvést širší kontext: průměrná zúčtovací cena na denním trhu za celý rok 2025 činila 96,96~EUR/MWh~\cite{ote-rocni-zprava}. Dosažená chyba 13~EUR/MWh tak představuje průměrnou odchylku zhruba 13~\% vůči celoročnímu průměru. XGBoost exceloval především v~predikcích nočních hodin.
...
Důležitým milníkem při tvorbě této práce bylo ověření funkčnosti a~smysluplnosti vytvořeného řešení experty z praxe. Velmi si proto vážím zpětné vazby, kterou mi poskytl Ing.~Jiří~Cigler,~Ph.D., technický ředitel společnosti Feramat~Energies,~s.r.o. Celé znění jeho odborného posudku je součástí této práce a~lze jej nalézt v příloze pod bodem \ref{itm:posudek}. Toto hodnocení potvrzuje, že zvolený postup – od využití moderních open-source nástrojů v Pythonu až po identifikaci klíčových tržních faktorů – odpovídá standardům běžně využívaným v komerční praxi.
...
\begin{enumerate}
  ...
  \item \label{itm:posudek} Odborný posudek od Ing. Jiřího Ciglera, Ph.D. (přiložen jako samostatný soubor \texttt{JiriStodulka-doporuceni.pdf}) 
\end{enumerate}
